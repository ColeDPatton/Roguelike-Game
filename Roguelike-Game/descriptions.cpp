...
#define NUM_MONSTER_DESCRIPTION_FIELDS 10
                              ...
                              uint32_t *integer)
{
  ...
  if (sscanf(lookahead->c_str(), "%d", integer) != 1) {
    return 1;
  }
...
static uint32_t parse_monster_lvl(std::ifstream &f,
                                   std::string *lookahead,
                                   uint32_t *level_required)
{
  return parse_integer(f, lookahead, level_required);
}
                                          ...
                                          std::vector<monster_description> *v)
{
  ...
  bool read_name, read_symb, read_color, read_desc,
    read_speed, read_dam, read_hp, read_abil, read_rrty, read_lvl_req;
  ...
  read_name = read_symb = read_color = read_desc = read_speed
            = read_dam = read_hp = read_abil = read_rrty = read_lvl_req = false;
  ...
  for (f >> *lookahead, count = 0;
    ...
    if        (*lookahead == "NAME")  {
      ...
      read_dam = true;
    }  else if (*lookahead == "LEVELREQUIRED")   {
      if (read_lvl_req || parse_monster_lvl(f, lookahead, &level_required)) {
        std::cerr << "Discovered at " << __FILE__ << ":" << __LINE__ << "\n"
                  << "Parse error in monster level.\n"
                  << "Discarding monster." << std::endl;
        return 1;
      }
      read_lvl_req = true;
  ...
  m.set(name, desc, symb, color, speed, abil, hp, dam, rrty, level_required);
  v->push_back(m);
...
uint32_t levelCheck(uint32_t mon_level, uint32_t dun_level){
  if(mon_level <= dun_level)
    return 1;
  return 0;
}
...
npc *monster_description::generate_monster(dungeon *d)
{
  ...
  while (!v[(i = (rand() % v.size()))].can_be_generated() ||
	 ...
	 !levelCheck(v[i].get_level_required(), d->level))
    ;
