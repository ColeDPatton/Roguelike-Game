...
void io_display(dungeon *d)
{
  ...
  * not otherwise used.                                                 */
  ...
  for(i=0;i<num_eq_slots;i++){
    if(d->PC->eq[i]){
      min_damage += d->PC->eq[i]->get_damage_base();
      max_damage += d->PC->eq[i]->get_damage_base();
      min_damage += (d->PC->eq[i]->get_damage_number() * 1);
      max_damage += (d->PC->eq[i]->get_damage_number() * 
      		     d->PC->eq[i]->get_damage_sides());

    }
  }
  mvprintw(23, 1, "PC stats[Attack:%d-%d Health:%d Speed:%d Level:%d]",
	   min_damage, max_damage, d->PC->hp, d->PC->speed, d->PC->pcLevel);
...
void io_spend_skill_point(dungeon* d){
  uint32_t key = 0;

  if(d->PC->skill_points > 0){
    mvprintw(5, 8, "%-58s ", "Skill Points left: ");
    mvprintw(5, 26, "%d ",d->PC->skill_points);
    mvprintw(6, 8, "%-58s ", "Click 1-4 to spend a skill point. ");
  }else
    mvprintw(6, 8, " %-58s ", "No skill points available");

  mvprintw(7, 8, " %-58s ", "1. Strength: ");
  mvprintw(7, 21, " %d", d->PC->str);
  mvprintw(8, 8, " %-58s ", "2. Constitution: ");
  mvprintw(8, 25, " %d", d->PC->cons);
  mvprintw(9, 8, " %-58s ", "3. Dextarity: ");
  mvprintw(9, 21, " %d", d->PC->dex);
  mvprintw(10, 8, " %-58s ", "4. Sight: ");
  mvprintw(10, 18, " %d", d->PC->sight);

  if(d->PC->skill_points == 0){
    key = getch();
  }
  
  while (d->PC->skill_points > 0) {
    if ((key = getch()) == 27 /* ESC */) {
      io_display(d);
      return;
    }else if(key>='1' && key<='4' && d->PC->skill_points > 0){
      d->PC->skill_points--;
      mvprintw(5, 26, "%d ",d->PC->skill_points);
      if(key == '1'){
	d->PC->str++;
	...
	d->PC->cons++;
	d->PC->max_hp += (d->PC->max_hp*(d->PC->cons/4));
	mvprintw(8, 25, " %d", d->PC->cons);
      }else if(key == '3'){
	d->PC->dex++;
	mvprintw(9, 21, " %d", d->PC->dex);
	d->PC->recalculate_speed(d);
      }else if(key == '4'){
	d->PC->sight++;
	mvprintw(10, 18, " %d", d->PC->sight);
      }
