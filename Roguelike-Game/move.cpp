  ...
  if (character_is_alive(def)) {
    if (atk != d->PC) {
      ...
      for (i = damage = 0; i < num_eq_slots; i++) {
        ...
        bonus_damage = ((damage/10) * (d->PC->str));
	...
	d->PC->currentXP += 
	  ((100-(((npc*)def)->md.get_rarity()))*
	   (((npc*)def)->md.get_level_required()));
        io_queue_message("%s%s dies.", is_unique(def) ? "" : "The ", def->name);
	if(d->PC->currentXP > (d->PC->pcLevel * 100)){
	  d->PC->pcLevel++;
	  d->PC->skill_points++;
	  d->PC->currentXP = 0;
	  io_queue_message("You leveled up to lvl %d!!!", d->PC->pcLevel);
	}
      }
      def->hp = 0;
      def->alive = 0;
...
uint32_t move_pc(dungeon *d, uint32_t dir)
{
  ...
  case '<':
    ...
    break;
  case '>':
    if (mappair(d->PC->position) == ter_stairs_down && (d->level > 1)) {
      was_stairs = 1;
      d->level--;
      new_dungeon_level(d, '>');
    }
