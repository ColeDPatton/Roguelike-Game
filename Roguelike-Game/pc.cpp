...
void pc_observe_terrain(pc *p, dungeon *d)
{
  ...
  y_min = p->position[dim_y] - p->sight;
  if (y_min < 0) {
    y_min = 0;
  }
  y_max = p->position[dim_y] + p->sight;
  if (y_max > DUNGEON_Y - 1) {
    y_max = DUNGEON_Y - 1;
  }
  x_min = p->position[dim_x] - p->sight;
  if (x_min < 0) {
    x_min = 0;
  }
  x_max = p->position[dim_x] + p->sight;
...
void pc::recalculate_speed(dungeon* d)
{
  int i;
